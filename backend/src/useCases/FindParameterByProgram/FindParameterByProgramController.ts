...
export class FindParameterByProgramController {
  constructor(private findParameterByProgramUseCase: FindParameterByProgramUseCase) {}
  ...
  async handle(request: Request, response: Response): Promise<Response> {
    ...
    try {
      const parameters = await this.findParameterByProgramUseCase.execute(programid)
